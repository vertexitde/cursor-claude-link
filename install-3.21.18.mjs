...
if(version!=='3.21.18'||product.commit!=='c4730f7d93d787d9ab120af715999f0345ee5bc0')throw new Error('This patch supports Cursor 3.21.18 build c4730f7 only.');
...
for(const surface of ['desktop','glass']){
  ...
  source=patchPickerSections(source,once,desktop
    ?{groupReturn:'return c.mergeLeadingIntoPromotedSection===!0?{leading:[],promoted:[...ie,...J],others:le}:{leading:ie,promoted:J,others:le}',
      promotedAnchor:'GP(omt,{models:B.promoted,title:c?.promotedSectionTitle',modelsVar:'B',jsx:'GP',fmt:'omt',renderModel:'x'}
    :{groupReturn:'return l.mergeLeadingIntoPromotedSection===!0?{leading:[],promoted:[...te,...X],others:ie}:{leading:te,promoted:X,others:ie}',
      promotedAnchor:'Y5(t9t,{models:N.promoted,title:l?.promotedSectionTitle',modelsVar:'N',jsx:'Y5',fmt:'t9t',renderModel:'w'});
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=l6d(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=Tum(u,';
  ...
  const native=desktop?'qh(this.storageService,"useDedicatedLocalAgentRuntimeHost")':'Rp(this.storageService,"useDedicatedLocalAgentRuntimeHost")';
  source=once(source,native,'(__isClaudeBridgeModel('+(desktop?'g':'p')+')&&Boolean(this.environmentService.remoteAuthority)||'+native+')');
  const activation=desktop?'function rhp(e){return Gc.localMode&&e?.get(Uoy,-1)==="true"}':'function uNg(t){return Rl.localMode&&t?.get(bNg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'KIy',useState:'Zur',useEffect:'iIy',card:'sb',zs:'Ts',bar:'kA',barStyle:'rdr',
      fn:'function eAy(e){const t=Bbp(119)',
      children:'title:"Plan & Usage",children:[ln,Gt,dn,vn]',
      childrenGpt:'title:"Plan & Usage",children:[ln,Gt,dn,vn,KIy(__chatgptUsageSection,{})]'}
    :{jsx:'Rik',useState:'gds',useEffect:'Aik',card:'Pf',zs:'Ns',bar:'Fm',barStyle:'IEi',
      fn:'function Dik(t){const e=bBg(119)',
      children:'title:"Plan & Usage",children:[pt,bt,gt,St]',
      childrenGpt:'title:"Plan & Usage",children:[pt,bt,gt,St,Rik(__chatgptUsageSection,{})]'};
  ...
  source=patchSubagentBubbles(source,surface,'3.21.18');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.21.18');
...
if(process.argv.includes('--check')){console.log('Cursor 3.21.18 Claude patch candidates passed syntax and anchor checks.');process.exit();}
