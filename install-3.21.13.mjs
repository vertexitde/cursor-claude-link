...
if(version!=='3.21.13'||product.commit!=='e44a49c17e334d442e58bbde931d791200f014a0')throw new Error('This patch supports Cursor 3.21.13 build e44a49c only.');
...
for(const surface of ['desktop','glass']){
  ...
  const activation=desktop?'function nhp(e){return zc.localMode&&e?.get(Ooy,-1)==="true"}':'function lNg(t){return Rl.localMode&&t?.get(fNg,-1)==="true"}';
  ...
  source=patchSubagentBubbles(source,surface,'3.21.13');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.21.13');
...
if(process.argv.includes('--check')){console.log('Cursor 3.21.13 Claude patch candidates passed syntax and anchor checks.');process.exit();}
