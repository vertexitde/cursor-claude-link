...
if(version!=='3.21.16'||product.commit!=='8ae78e8eee1e63479c7e0504b664bc0a80c68000')throw new Error('This patch supports Cursor 3.21.16 build 8ae78e8 only.');
...
for(const surface of ['desktop','glass']){
  ...
  if(source.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
    ...
    const before=desktop?'async run(e,t,n,i,r,s,o,a,c,l,u){const h=c6d(u,':'async run(t,e,n,i,r,s,o,a,l,c,u){const d=Cum(u,';
  ...
  const activation=desktop?'function ihp(e){return Gc.localMode&&e?.get(Boy,-1)==="true"}':'function cNg(t){return Rl.localMode&&t?.get(vNg,-1)==="true"}';
  ...
  const usage=desktop
    ?{jsx:'qIy',useState:'Zur',useEffect:'nIy',card:'sb',zs:'Ts',bar:'kA',barStyle:'rdr',
      fn:'function JIy(e){const t=Fbp(119)',
      children:'title:"Plan & Usage",children:[ln,Gt,dn,vn]',
      childrenGpt:'title:"Plan & Usage",children:[ln,Gt,dn,vn,qIy(__chatgptUsageSection,{})]'}
    :{jsx:'Aik',useState:'gds',useEffect:'Iik',card:'Pf',zs:'Ns',bar:'Fm',barStyle:'IEi',
      fn:'function Mik(t){const e=vBg(119)',
      children:'title:"Plan & Usage",children:[pt,bt,gt,St]',
      childrenGpt:'title:"Plan & Usage",children:[pt,bt,gt,St,Aik(__chatgptUsageSection,{})]'};
  ...
  source=patchSubagentBubbles(source,surface,'3.21.16');
  source=patchSubagentLifecycle(source,surface,'claude-subscription/','3.21.16');
...
if(process.argv.includes('--check')){console.log('Cursor 3.21.16 Claude patch candidates passed syntax and anchor checks.');process.exit();}
